					...
					<ul class="inps_blkslog">
						<li>
							<div class="log_inp_blk">
								<input type="email" name="email" class="input_control" id="email" autocomplete="false">
								<label class="log_lbl"> Enter email </label>
							</div>
						</li>
						<li>
							<div class="log_inp_blk">
								<input type="password" name="password" id="password" class="input_control">
...
<script type="text/javascript">
	...
	function checkEmail()
	{
		...
		if(!email)
		{
			...
			$("#email").val('');
			//$("#message").html("Please Enter Your Registered Email");
			return false;
		}
			...
			$("#email").val('');
			//$("#message").html('"'+email+'" is not Valid Email'); 
			return false;
		} 
		else
		{
			$.ajax({
				...
				success : function(response)
				{
					...
					else
					{
						...
						$("#email").val('');
						//$("#message").html('"'+email+'"is not registered Email');
						return false;
					}
				}
			});
			...
			$.ajax({
				...
				success : function(response)
				{
					...
					if(res.status == 'error')
					{
						...
						//$("#message").html('Authentication Failed');
						return false;
					}
					else
					{
