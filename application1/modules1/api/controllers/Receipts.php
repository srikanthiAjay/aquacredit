	...
	public function get_receipts()
	{
		...
		if(count($receipts)>0)
		{
			foreach($receipts as $r) {
				...
				$data[] = array(					
					'<a class="edt" href="#" onclick="edit_receipt('.$r["rc_id"].');"> RCP'.$r["rc_id"].' </a>',
					...
					'<i class="fa fa-ellipsis-v act_icn" data-toggle="popover" data-placement="left" tabindex="0" data-trigger="focus" aria-hidden="true" onclick="edit_receipt('.$r["rc_id"].');"></i>'
				);
			}
		}
	...
	public function add()
	{
		...
		if($final_res["status"] == "success")
		{
			if($_POST["trans_type"] == "bank")
			{
				...
				$this->db->where('bank_id', $admin_bank);
