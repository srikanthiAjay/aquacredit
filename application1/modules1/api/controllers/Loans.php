		...
		if(count($loans)>0)
		{
			foreach($loans as $r) {
			   ...
			   $data[] = array(	'<input type="checkbox" onclick="stopPropagation(event)">',				
					'<a class="edt" href="javascript:void(0);" onclick="loan_upd('.$r["loan_id"].','.$r["status"].');"> LN'.$r["loan_id"].' </a>',
					$approve_date,
	...
	public function update()
	{
		...
		{		
			...
			if($res["status"] == "success")
			{
				if($_POST["act_types_edit"]=="bank")
				{
					$upd_bank = $this->Loans_model->updateAdminBankAmount($_POST["admin_bank"],$_POST["loan_amt_edit"]);
				}
				else if($_POST["act_types_edit"]=="cash")
				{
					$admin_id = $this->session->userdata('adminid');
					...
					$final_amt = $branch_data["data"]["amount"] - $_POST["loan_amt_edit"];
