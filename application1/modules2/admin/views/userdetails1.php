...
<div class="right_blk">
	<div class="top_ttl_blk">
		...
		<span> <b><?php echo $user["user_name"];?> </b> Transactions - #AU000001 </span>
		...
		<div class="main_anal">
			...
			<ul class="anl_tcs">
				...
				<li class="bor_lf_none">
					<div class="top_in_op crop_top">
						<p>Harvest </p>
						<h1> ₹<?php echo $totalHarvest;?> </h1>
					</div>
				</li>
				<li class="bor_lf_none">
					<div class="top_in_op crop_top">
						<p>Acres </p>
						<h1> <?php echo $totalAcres;?> </h1>
					</div>
				</li>
			</ul>
		</div>
          				...
          				<tbody>
           				...
           				<tfoot>
							...
							<tr>
								...
								<td class="txt_rt" id="open_bal"> +0 </td>
							</tr>
							<!-- <tr>
								<td class="opic_non date"> &nbsp;  </td>
							...
							<tr>
								...
								<td class="grd_ttl txt_rt grand_total" id="grand_total"> <b>+0</b> </td>
							</tr>
						</tfoot>
					</table>
				</div>
...
function load_unsettled()
{
	...
	var tables = $('#usr_lst_tbl').DataTable({
		...
		'ajax': {
			...
			"dataSrc": function (json) {
				...
				$(".in_td").html('₹'+addCommas(total_trans_amount));
				$("#open_bal").html('₹'+addCommas('0'));
				$(".grand_total").html('₹'+addCommas(total_trans_amount));		
				return json.data;
			}
		},
		"footerCallback": function (tfoot, data, start, end, display) {
			...
			$footer_html ='<tr>'+
							...
							'<tr>'+
								...
								'<td class="grd_ttl txt_rt grand_total" id="grand_total"> <b>+0</b> </td>'+
							'</tr>';
			$(tfoot).find('tfoot').eq(0).html($footer_html);
	...
	//$(document).on("click", ".check_list input[name='crop_opt']", function(){
...
function load_settled()
{
	...
	var tables = $('#usr_lst_tbl').DataTable({
    	...
    	'serverMethod': 'post',
		"columns": [
			{title: "", className: "","width": "10%"},
			{title: "Settled Date", className: "date","width": "20%"},
			{title: "Settled Id",className: "", "width": "20%"},
			{title: "Settled Amount",className: "txt_rt out_td","width": "40%"},
			{title: "Balance",className: "txt_rt out_td","width": "10%"},
		],
	...
	console.log('gh');
	$('.dataTables_scrollBody').css('height', min_h);
	$('#usr_lst_tbl_wrapper .dataTables_length').html('<ul class="tabs_tbl"><li class="act_tab drft_cl" id="status_0"> <span>Unsettled</span> </li><li class="act_tab comp_cl" id="status_1"> <span>Settled </span> </li></ul> <span class="tbl_btn">  </span>');
	//$(document).on("click", ".check_list input[name='crop_opt']", function(){
	$('#usr_lst_tbl tbody').on('click', '.expand_details', function () {
		console.log('expand on click');
        var tr = $(this).closest('tr');
...
function load_analytics()
{
	$.ajax({
		...
		success: function(data){
			...
			$("#cropLoan").text('₹'+addCommas(data.cropLoan));
			$("#cropOrder").text('₹'+addCommas(data.cropOrders));
			$("#cropHarvest").text('₹'+addCommas(data.cropHarvest));
				 ...
				 details = '<tr class="detal_row">'+
					...
					$.each(data.data, function(key,value) {
						details += '<tr>'+
									...
									'<td> '+'₹'+addCommas(value.mrp)+' </td>'+
									'<td> '+value.discount+' </td>'+
									'<td> '+'₹'+addCommas(value.total_price)+' </td>'+
								'</tr>';
					}); 
				 ...
				 details = '<tr class="detal_row">'+
					...
					$.each(data.data, function(key,value) {
						details += '<tr>'+
									...
									'<td> '+value.count+' </td>'+
									'<td> '+'₹'+addCommas(value.farmer_price)+' </td>'+
									'<td> '+value.farmer_weight+' </td>'+
									'<td> '+'₹'+addCommas(value.farmer_amount)+' </td>'+
								'</tr>';
					}); 
