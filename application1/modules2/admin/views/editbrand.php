...
<div class="right_blk">
	<div class="top_ttl_blk"> 
		<span class="back_btn"><!-- <a href="<?php echo base_url();?>admin/Companies" title=""><img src="<?php echo base_url();?>assets/images/back.png" alt="" title=""> </a> --></span> 
				...
				<form action="javascript:void(0);" id="add_brand" name="add_brand" method="post" >
					<div class="row"> 
						...
						<div class="col-md-4"> 
							...
							<input type="text" class="form-control" id="brand_name" name="brand_name" value="" placeholder="Company Name" onblur="checkbrandname();">
							</div> 
						</div>
...
function getbrand()
{
	...
	$.ajax({		
		url: url+"api/Companies/index/"+bid,
		data: {},
		type:'POST',		
		datatype:'json',
...
function allcats()
{
	...
	$.ajax({		
		//url: url+"admin/Companies/allcats",
...
{	
	...
	$.ajax({		
		url: url+"api/Companies/getSubCat",
		data: {catid:value},
		type:'POST',		
		datatype:'json',
		success : function(response){
...
function fromsbmt(frm){
	...
	if(catexists == 0){		
		...
		$.ajax({
			url: url+"api/Companies/cat_subcat_add",
			data: formData,
			type:'POST',
			contentType: false,
			processData: false,
...
{	
	...
	if(catname !=""){ 
		$.ajax({		
			url: url+"api/Companies/checkcategory",
...
function checkbrandname()
{
	...
	if(brandname !=""){ 
		...
		{
			$.ajax({		
				url: url+"api/Companies/checkbrandname",
