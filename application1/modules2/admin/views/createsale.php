				...
				<tbody id="invoiceItem">
					...
					<tr id="rowNums0"> 
						...
						<input type="hidden" placeholder="0" name="prototval[]" id="prototval0" >
						 ...
						 <td><i class="fa fa-trash" onclick="removerow(0)" style="color:red"></i></td>
					</tr>
				</tbody>
				<tfoot>
					<tr>
    ...
    success : function(response){
      ...
      res= JSON.parse(response);        
        ...
        if(res.data.length > 0)
        {
            ...
            opt += '<div class="form-check"><input class="form-check-input" type="radio" name="branchval" id="branch'+index+'" value="'+crop.branch_id+'" /><label class="form-check-label" for="branch'+index+'">'+crop.branch_name+'</label></div>';
          });
        }
...
$("#salefrm").submit(function(e) {  
  ...
  /* print_preview */
  $('#invoice_body').load("<?php echo base_url('api/sales/invoice_preview')?>", $("form").serializeArray()
	);
...
function addmorerows()
{
	var rr = $('#rowval').val();
			...
			var rowNum = rr;
	     	...
	     	htmlRows = '<tr id="rowNums'+rowNum+'" ><td><input type="hidden" placeholder="Product Name" name="proid[]" id="proid'+rowNum+'" value="0" ><input class="mykey" type="text" placeholder="Product Name" autocomplete="off" name="proname[]" id="proname'+rowNum+'" > </td><td class="qty txt_cnt"> <input type="text" class="mykey" placeholder="0" onkeypress="return onlyNumberKey(event)" name="proqty[]" id="proqty'+rowNum+'"></td><td class="mrp txt_rt"> <input type="text" class="mykey" readonly placeholder="0" name="promrp[]" id="promrp'+rowNum+'"><input type="hidden" placeholder="0" name="promrpval[]" id="promrpval'+rowNum+'"> </td><td class="disc txt_rt"> <input type="text" class="mykey disckey" placeholder="0" name="prodisc[]" id="prodisc'+rowNum+'"> </td><td class="ttl_prc txt_rt"> <input type="text" placeholder="0" readonly name="protot[]" id="protot'+rowNum+'"><input type="hidden" placeholder="0" name="prototval[]" id="prototval'+rowNum+'" > </td><td ><i class="fa fa-trash" onclick="removerow('+rowNum+')" style="color:red" ></i></td></tr>';
