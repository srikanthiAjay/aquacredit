...
class Trades_model extends CI_Model 
{
	...
	{
		$data = $this->db->select('DATE_FORMAT(trade_date, "%d-%b-%Y") as trade_date,count,farmer_price,farmer_weight,farmer_amount,company_price,company_weight,company_amount')->get_where("trade_actual_details", ['trade_id' => $trade_id])->result();
		return json_encode(array('status'=>'success','data' => $data));
	}
