...
class Loans_model extends CI_Model 
{
	...
	{
		//$this->db->select('ac_loan_types.loan_type,sum(loan_details.loan_amt) as counts');
		if($ltype == "pamt" || $ltype == "tamt" || $ltype == "ramt"){
			
			$this->db->select('ac_loan_types.loan_type,sum(loan_details.loan_amt) as counts,status');
			
			if($ltype == "tamt"){ 
				$this->db->group_by('loan_type_id'); 
			}
			else if($ltype == "pamt" || $ltype == "ramt"){ 
	...
	{
		$response = array();
		$query = $this->db->query("SELECT *,(select count(loan_id) from loan_details) as tot_rec,(select count(loan_id) from loan_details where status = '0') as pending,(select count(loan_id) from loan_details where status = '1') as approved,(select count(loan_id) from loan_details where status = '2') as rejected,(select SUM(loan_amt) from loan_details where status = '1') as tot_amt,(select SUM(loan_amt) from loan_details where status = '0') as pending_amt FROM loan_details");
		//echo $this->db->last_query();
        if($query->num_rows()>0)
        {
