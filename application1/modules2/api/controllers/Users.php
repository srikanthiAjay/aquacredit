	...
	{		
		...
		if($total_count)
		{
			foreach($tansactions as $key=>$value){
				...
				{
					$amount -=$value['amount'];
					$amt = '<span class="txt_red">'.$value['amount'].'<div class="arr_blk"> <img src="http://3.7.44.132/aquacredit/assets/images/rd_c_ar.png"> </div></span>';
				}  
				else 
				{
					$amount +=$value["amount"];
					$amt = '<span class="grn_clr">'.$value['amount'].'<div class="arr_blk"> <img src="http://3.7.44.132/aquacredit/assets/images/grn_c_ar.png"> </div></span>';
				}
		...
		if($total_count)
		{
			foreach($tansactions as $key=>$value){
				if($value["balance_amount"] < 0) 
				{
					...
					$amt = '<span class="txt_red">Loss('.$value['balance_amount'].')</span>';
				}  
				else 
				{
					
					$amt = '<span class="grn_clr">Profit('.$value['balance_amount'].')</span>';
				}
				$trans = ($value['trans'] != null) ? "(".$value['trans'].")" : "";
				$data[]=[
	...
	public function getSettledDetails($settled_id)
	{
		...
		if($total_count)
		{
			...
			foreach($tansactions as $key=>$value){
				...
				{
					$trans = ($value['trans'] != null) ? "(".$value['trans'].")" : "";
					$tr_html .= "<tr><td></td><td>".date("d-M-Y",strtotime($value['created_on']))."</td><td>".$value["trans_type"].' '.$trans.' - '.$value["trans_code"]."</td><td>".$value["amount"]."</td></tr>";
				...
				else if($value["trans_type"]=="SALE" && $value["trans"]=="GOADS")
				{
					$tr_html .= "<tr><td></td><td>".date("d-M-Y",strtotime($value['created_on']))."</td><td>".$value["trans_type"].' '.$trans.' - '.$value["trans_code"]."</td><td>".$value["amount"]."</td></tr>";
						...
						"<td colspan='2'>".
							...
							foreach($trades["data"] as $trade){
								...
								$tr_html .= "<tr>".
									...
									"<td> ".$trade["count"]." </td>".
									"<td> ".$trade["farmer_price"]." </td>".
									"<td>  ".$trade["farmer_weight"]."  </td>".
									"<td>  ".$trade["farmer_amount"]."  </td>".
								"</tr>";
							}
							$tr_html .="</table>".
								"</td>".
