...
class Traders extends CI_Controller 
{
	...
	public function get_traders()
	{
		...
		if(count($traders)>0)
		{
			foreach($traders as $r) {
				...
				$balance = IND_money_format($r["balance"]);
			   $data[] = array(					
					//$r["trader_code"],
					'TDR'.$r["td_id"],
					...
					$r["trader_type"],
					'₹'.$balance,
					'<i class="fa fa-ellipsis-v act_icn" data-toggle="popover" data-placement="left" tabindex="0" data-trigger="focus"  aria-hidden="true" onclick="edit_trader('.$r["td_id"].');"></i>'
			   );
			}
		}
	...
	public function transactions()
	{
		...
		if($total_count)
		{
			foreach($tansactions as $key=>$value){
				...
				{
					$amount -=$value['amount'];
					$amt = '<span class="txt_red">'.IND_money_format($value['amount']).'<span class="arr_blk"> <img src="http://3.7.44.132/aquacredit/assets/images/rd_c_ar.png"> </span></span>';
				}  
				else 
				{
					$amount +=$value["amount"];
					$amt = '<span class="grn_clr">'.IND_money_format($value['amount']).'<span class="arr_blk"> <img src="http://3.7.44.132/aquacredit/assets/images/grn_c_ar.png"> </span></span>';
				}
				$trans = ($value['trans'] != null) ? "(".$value['trans'].")" : "";
				$data[]=[
								date("d-M-Y",strtotime($value['created_on'])),
								'<a href="javascript:void(0);" class="expand_details" title="">'. $value["trans_type"].' '.$trans.' - '.$value["trans_code"].'</a> <i class="fas fa-eye" onclick="load_'.$value["trans_type"].'('.$value['trans_id'].')"></i>',
								$amt,
								$value["trans_type"],
								$value['trans'],
								$value['trans_id'],
	...
	public function printAgentTrans($trader_id)
	{
		...
		//exit;
		$time = time();
		$pdfFilePath = $_SERVER['DOCUMENT_ROOT']."/credit_new/assets/pdf/transactions/trans_".date('m-d-Y_hia').".pdf";	
