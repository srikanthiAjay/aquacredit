$(document).ready(function() {
	...
	getadminbranches('both',0); $(".unload_block").hide(); $(".dest_trans_block").hide(); $(".rc_btn").hide();
...
{		
	...
	if(addoredit == "edit")
	{
		...
		if(addoredit == "add")
		{
			...
			if(last_id == undefined ){ instance = 0;}
			else{ instance++;}
...
function edit_stock_trans(stk_trans_id,lstatus)
{
	...
	$('#sa_action').val('');
	if(lstatus == 1){ 
		$(".sb_btn").hide(); $("#add_row").hide(); $(".rc_btn").hide();
		$('#stk tr td:last-child,#stk tr th:last-child').hide();}
	else{ $(".sb_btn").show(); $("#add_row").show(); $(".rc_btn").show();}
