$(document).ready(function() {
	...
	getadminbanks('add'); $(".guest_block").hide(); $(".guest_block_edit").hide();
	...
	$('.rec_sen_btn').click(function(){
		...
		if(receipt_date == ""){ err = 1; err_msg = "Please select receipt date!"; 
			tagid = "#receipt_date"; 
		...
		if(rece_amt == ""){
			err = 1; err_msg = "Please enter receipt amount!"; 
		...
		if(user_type == "FARMER" && guest == 0)
		{
			if(crop_opt == undefined){
				err = 1; err_msg = "Please select crop location!"; 
				tagid = ".crop_opt"; 
				return form_validation(err,err_msg,tagid);
			}
		}	
...
// <a href="#" class="appr_all"> Approve All </a>
   $(".loan_btm div.toolbar").html('<b>SSS</b>');
	...
	$('.utypes').show(); $('.ttypes').hide();
	$('.comp_cl').click(function(){
		$("#hid_tabval").val(1);
		$('.utypes').hide(); $('.ttypes').show();
...
function edit_receipt(rc_id)
{
	...
	$.ajax({		
		...
		success : function(response){
			...
			$(".admin_bank_val_edit").text(res.data.account_no);
			$('input[name="admin_bank_edit"][value="'+res.data.admin_bank_id+'"]').prop('checked', true);
		...
		success : function(response){
			...
			if(res.data.length > 0)
			{
				$.each(res.data, function(index, bank) {
					...
					opt += '<div class="form-check"><input class="form-check-input" type="radio" name="admin_bank'+aeval+'" id="bnk'+index+aeval+'" value="'+bank.bank_id+'"><label class="form-check-label" for="bnk'+index+aeval+'"><div class="bank_logo"> <img src="http://3.7.44.132/aquacredit/assets/images/'+bank_icn+'" alt="" title=""> </div><div class="bank_mny"><div class="bank_bal"> ₹ '+addCommas(bank.avail_amount)+' </div><div class="accont_numb">'+bank.account_no+'</div></div></label></div>';
				});
			}
			$(".admin_bank_val"+aeval).text('Select Admin Bank');
