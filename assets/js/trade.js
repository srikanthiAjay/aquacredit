    ...
    $("#ukey").autocomplete({
        ...
        select: function(event, ui) {
            ...
            if (trade_type == 'guest') {
                ...
                $("#mobile").parent().css("border", "");
                $("#mobile").parent().addClass("inp_ss");
            }
        ...
        success: function(response) {
            ...
            $.ajax({
                ...
                success: function(response1) {
                    ...
                    if (rescp1.status == "success") $("#loading").hide();
                    var aeval = '_edit';
                    var opt = '';
                    if (rescp1.data.length > 0) {
                        $.each(rescp1.data, function(index, crop) {
                            if (crop.cd_id == location) {
                                sel = "checked";
                            ...
                            opt += '<div class="form-check"><input class="form-check-input" type="radio" name="crop_opt_edit" id="crp' + index + aeval + '" value="' + crop.cd_id + '" ' + sel + ' required/><label class="form-check-label" for="crp' + index + aeval + '">' + crop.crop_location + '</label></div>';
                        });
                        $("#crop_opt_li_edit").html(opt);
                    } else {
                        $("#crop_opt_li_edit").html('<div class="form-check"><input class="form-check-input" type="radio" name="crop_opt_edit" id="crp" required /><label class="form-check-label" for="crp"></label></div>');
...
function calculateTotal1() {
    ...
    var exp = parseFloat(expenses) + parseFloat(labfee);
    if (exp > 0) {
        $(".us_bn_ls").show();
    } else {
        $(".us_bn_ls").hide();
...
$(document).ready(function() {
    ...
    var validator = $('#tradefrm').validate({
        debug: false,
        ignore: '',
        rules: {
            tkey: {
            ...
            mobile: {
                required: function(element) {
                    return ($("input[name='trade_type']:checked").val() == "guest") ? true : false;
                },
                minlength: 10,
                maxlength: 10,
                remote: {
    ...
    $(document).on("click", ".save_lnk", function() {
        ...
        $('#tradefrm_edit_exp').validate({
            ...
            rules: {
                ...
                crop_opt_edit: {
                    required: function(element) {
                        return ($("#trade_type_edit").val() == "0");
                    }
                },
                mobile_edit: {
                    required: function(element) {
                        return ($("#trade_type_edit").val() == "1") ? true : false;
                    },
                    minlength: 10,
                    maxlength: 10,
                },
    ...
    $("#tradefrm_edit").submit(function(e) {
        ...
        if ($('#status').val() == "1") {
            var err = 0;
            if ($("#expenses").val() > 0 || $("#labfee").val() > 0) {
                if ($("input[name='admin_bank']:checked").val() == undefined)
                    alert("Please Select Admin Account");
            }
    ...
    $(document).on('blur', '#ukey_edit', function() {
        $(".crop_type_val").html('Crop location');
        if ($(this).val() == '') {
            $(".userid_edit").val('');
        }
