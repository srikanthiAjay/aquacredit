$(document).ready(function() {
	...
	$("#recdate").datepicker({
        ...
        onSelect: function(selected) {
            ...
            $("#recdate").removeClass('error');
            str = selected.split("-").join(" ");
            var dt = new Date(str);
            //dt.setDate(dt.getDate() + 1);
            $("#retdate").datepicker("option", "minDate", dt);
        }
    });
	$("#retdate").datepicker({
        dateFormat: 'dd-M-yy',
        ...
        onSelect: function(selected) {
            //event.stopPropagation();            
            $("#retdate").removeClass('error');
            str = selected.split("-").join(" ");
            var dt = new Date(str);
            //dt.setDate(dt.getDate() + 1);
            $("#recdate").datepicker("option", "maxDate", dt);
        }
    });
        ...
        $('#farmer').submit(function(e) {
            ...
            rules: {
                ...
                mobile: {
                    ...
                    maxlength: 10,
					remote: {
						url: url + "api/users/checkmobile_exists_tool_tip",
						type: "post",
						data: {
							mobnum: function() {
				...
				"ac_number[]":{
					remote: {
						url: url + "api/Users/check_accno_for_tooltip",
						type: "post",
						data: {
							acc_no: function() {
								return $('#farmer :input[name="ac_number[]"]').val();
							}
						}
					}
				},
            ...
            submitHandler: function(form) {
               ...
               /*  if (mobile_exist == 1) {
                ...
                if (actiontype == 'add') {
                    ...
                    var action_url = url + "api/users/createUser";
