...
class Receipts_model extends CI_Model 
{
	...
	function getReceiptDetails($rc_id)
	{
		...
		$this->db->select('receipts.*,users.user_name,users.owner_name,users.user_code,users.mobile,users.user_type,users.typeofuser,user_crop_details.cd_id,user_crop_details.crop_location,accounts.account_name,accounts.account_number,traders.td_id,traders.trader_type,traders.contact_person,traders.full_name');
		$this->db->join('accounts', 'accounts.id = receipts.admin_bank_id','left');
	...
	function receipts_search($limit,$start,$tabval,$search,$reportRange,$trans,$utype_opt,$col,$dir)    
    {
		...
		if($utype_opt == "farmer")
		{
			$where .= " AND users.user_type LIKE 'FARMER' AND users.typeofuser = '0' ";
		...
		}else if($utype_opt == "guest")
		{
			$where .= " AND users.user_type LIKE 'FARMER' AND users.typeofuser = '1' ";
