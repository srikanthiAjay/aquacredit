...
class Users extends CI_Controller 
{
	...
	{
		if(isset($_POST["aore"])){ $aore = trim($_POST["aore"]); }else{ $aore = "";}
		if(isset($_POST["uid"])){ $uid = trim($_POST["uid"]); }else{ $uid = "";}
		$mobnum=trim($_POST["mobnum"]); 
		$res=$this->Users_model->checkMobile_new($mobnum,$aore,$uid);		
	...
	/* public function uploadImage($imgfile,$usercode)
	{
		...
		for($i=0; $i< $files_count; $i++){
			...
			$upload_dir = FCPATH.'assets'. DIRECTORY_SEPARATOR . 'upload_docs'. DIRECTORY_SEPARATOR . $usercode;
			//$upload_dir = base_url().'assets/upload_docs/'. $usercode;
	...
	{	
		 //print_r($_POST);exit;
		$final_res = json_decode($this->Users_model->check_user_accno($_POST["acc_no"]),true);
	...
	public function createUser(){
		...
		//echo $recdate = date('Y-m-d',strtotime($_POST['recdate']));
		//echo $retdate = date('Y-m-d',strtotime($_POST['retdate']));
		//print_r($_POST);exit;
	...
	public function primary_bank()
	{
		...
		$res = $this->Users_model->updateBankDetails($_POST["prev_bank"],$pre_post);
		$primary_post = array('status' => 1);
		echo $response = $this->Users_model->updateBankDetails($_POST["acc_id"],$primary_post);
		
	}
	public function banks($user_id)
	{		
		echo $response = $this->Users_model->getUserBankAcc_new($user_id);
	}
	public function bank_delete()
	{
		$user_acc_posts = array('deleted' => 1);
		echo $response = $this->Users_model->updateBankDetails($_POST["acc_id"],$user_acc_posts);
