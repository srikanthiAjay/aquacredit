...
class SMS2
{
	...
	//OTP message for partner app
	public function otpSMSPartner($otp, $mobile,$type)
	{
		$smscontent = $this->getValue('ad_messages','message','action','otp_seller');
       	        $smscontent = str_replace("&otp&",$otp,$smscontent);
		$smscontent = urlencode($smscontent);
		$res = $this->send($smscontent,$mobile,$type);
		return $res;
	...
	{
		$name = ucfirst($name);
		$smscontent = "Dear $name, please complete your profile information at Aquadeals Seller App.";
		$smscontent = urlencode($smscontent);
	...
	public function book_rem_paid($book_id,$seller_id,$paid_amt,$type)
	{
		...
		$smscontent = urlencode($smscontent);
		$mobile = $this->getValue('ad_sellers','mobile','id',$seller_id);
		$res = $this->send($smscontent,$mobile,1);
		if($type == 0)
        {
        ...
        if($assigned_id !=0)
        {
            $amobile = $this->getValue('ad_access_sellers','mobile','id',$assigned_id);
            $res = $this->send($smscontent,$amobile,1);
        }
		return $res;
	}
...
//Manufacturer rejected message
	...
	public function dealBookConfiramtion($bookedId,$utype)
	{
		...
		$squery = $CI->db->query("SELECT * from ad_access_sellers where seller_id = $vendor_id and status=1 and (role=1 or role=2)");
	...
	public function productDealBookConfiramtion($bookedId,$utype)
	{
		...
		{  
                ...
                $squery = $CI->db->query("SELECT * from ad_access_sellers where seller_id = $manuf_id and status=1 and (role=1 or role=2)");
		...
		if($squery->num_rows() > 0)
		{
	        foreach($squery->result() as $row)
	        {
                if($row->id!=$check_id)
                {
                    ...
                    $Ssmscontent = str_replace("&quantity&",$quantity,$Ssmscontent);
                    $Ssmscontent = str_replace("&reason&",$creason,$Ssmscontent);
            		$Ssmscontent = str_replace("&reason&",$sad_helpline,$Ssmscontent);
                    $Ssmscontent = urlencode($Ssmscontent);
                    $res = $this->send($Ssmscontent,$row->mobile,1);
                }
	        }
	...
	function inform2Owner($mobile,$name,$amt,$type,$bid)
	{
		$smscontent='';
		if($type == 'Debit')
		{
		...
		$seller_id = 0;
                if($type==0)
                {
                        ...
                        if($sts=="process")
                        {
                                ...
                                $user_msg = "Your order(".$book_id.", ".$specie.") is processed.";
                                $status = "Order Processed";
                        }
                        if($sts=="shipping")
                        {
                                $msg = "Order $book_id is shipped";
                                if($dname!='')
                                {
                                        $user_msg = "Dear $uname, Your Order $book_id is shipped. Call AquaDeals $ad_helpline for help";
                                        
                                }
                                else
                                {
                                        $user_msg = "Dear $pname, Order $book_id is shipped";
                                }
                                $status = "Order Shipping Started";
                        }
                        if($sts=="deliver")
                        {
                                $msg = "Order $book_id is delivered";
                                if($dname!='')
                                {
                                        $user_msg = "Dear $uname, Your Order $book_id is delivered. Call AquaDeals $ad_helpline for help";
                                        
                                }
                                else
                                {
                                        $user_msg = "Dear $pname, Order $book_id is delivered";
                                }
                ...
                else
                {
                        ...
                        if($sts=="process")
                        {
                                $msg = "Your order $book_id confirmed now.";
                ...
                //Send sms to supervisor or manager
                $squery = $CI->db->query("SELECT * from ad_access_sellers where seller_id = $seller_id and status=1 and (role=1 or role=2)");
                if($squery->num_rows() > 0)
                {
                        foreach($squery->result() as $row)
                        {
	...
	function confirmOrder($bid,$sts,$type,$opt)
	{
	  ...
	  // echo "SELECT * from ad_access_sellers where seller_id = $seller_id and (role=1 or role=2)";exit;
		...
		if($assignto==1)
		{
                ...
                function sentLeadnotification($id,$did,$lid)
                {
                    $CI = & get_instance();
                    $query = $CI->db->query("SELECT contact_person,type,mobile from ad_sellers where id = $id");
                    $type = $query->row('type');
										$get_acc_persons = $CI->db->query("SELECT mobile,name FROM ad_access_sellers WHERE seller_id=$id and role=2 and status=1");
                    ...
                    $mobile = $query->row('mobile');
                    //$message = "Dear $name you got new best price request for $pname for more info login to app.";
										$message = "Dear $name, you got a new best price request for $pname. Respond to user by Login to Partner app.";
                    $smsContent = urlencode($message);
                    $this->send($smsContent,$mobile,0);
										if($get_acc_persons->num_rows()>0)
										{
											foreach($get_acc_persons->result() AS $row)
											{
												$acc_name = $row->name;
												$mob = $row->mobile;
												$msg = "Dear $acc_name, you got a new best price request for $pname. Respond to user by Login to Partner app.";
												$smsContent = urlencode($msg);
												$this->send($smsContent,$mob,0);
											}
										}
                }
                ...
                function convertOrder($bid,$type)
				{
		            ...
		            //Seller details
		            $old_seller_mbl = $this->getValue('ad_sellers','mobile','id',$old_id);
		            $new_seller_mbl = $this->getValue('ad_sellers','mobile','id',$new_id);
		            ...
		            $old_seller_msg = "Dear ".$old_seller.", your Order ".$book_id." is cancelled. Reason: Due to unavailability of partner.Call $ad_helpline for help";
		             $old_seller_sms = urlencode($old_seller_msg);
		            ...
		            $new_seller_sms  = urlencode($new_seller_msg);
		            
		            //Seller mobile numbers
		            
		            
		            $this->send($old_seller_sms ,$old_seller_mbl,1);  
		            $this->send($new_seller_sms ,$new_seller_mbl,1);  
				}
				function sendshipment_link($bid,$pay_link,$type)
                {
                    ...
                    //Send sms to user    
                    $user_msg = "Dear ".ucfirst($uname).", Your order $book_id is processed. You need to pay Rs.".$ship_charge." for shipping charges. Click on link to pay now ".$pay_link.". Call $ad_helpline for help";
                    $usermobile = $this->getValue('ad_users','mobile','id',$user_id);
                    $smscontent = urlencode($user_msg);
                    $res = $this->send($smscontent,$usermobile,0);
