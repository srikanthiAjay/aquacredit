...
class Users extends CI_Controller 
{
	...
	public function updatefinaldata()
	{
		...
		$act_res2 = $this->Transaction_model->Updatesettleaccount($activity_posts2,$_POST['userid'],$_POST['crop_id']);
	...
	public function saledataupdate()
	{
		...
		for($i=0;$i<count($_POST['brandid']);$i++)
		{
			...
			if(!empty($_POST['prodid'][$i]) && !empty($_POST['categoryid'][$i]) && !empty($_POST['brandid'][$i]) && !empty($_POST['branddiscount'][$i]) )
			{
				...
				{
					$queryb = $this->db->query("select *from dummy_account_sale where category_id='".$_POST['categoryid'][$i]."' and 	brand_id='".$_POST['brandid'][$i]."' and userid='".$_POST['auserid']."' and cropid='".$_POST['acropid']."' and product_id='".$_POST['prodid'][$i]."' ");
				}
				else
				{
					$queryb = $this->db->query("select *from dummy_account_sale where category_id='".$_POST['categoryid'][$i]."' and 	brand_id='".$_POST['brandid'][$i]."' and userid='".$_POST['auserid']."' and cropid='".$_POST['acropid']."' ");
				}
	...
	{
				$this->db->select('transactions.*, sale.id,sale.id as sid,sale.branchid as brnchid,sale_details.product_id,sale_details.mrp,sale_details.quantity,sale_details.brandid as brandid,products.cat_id,categories.cat_id as categoryid,categories.cat_name, sum(`products`.`pmrp`) as `totamt`');
				$this->db->join('sale', 'sale.id = transactions.trans_id','left');
				$this->db->join('sale_details', 'sale_details.s_id = sale.id','left');
				$this->db->join('products', 'products.pid = sale_details.product_id','left');
				$this->db->join('categories', 'categories.cat_id = products.cat_id','left');
				...
				foreach($dat1 as $daa)
				{
					...
					/*data*/
					$this->db->select('transactions.*, sale.id,sale.id as sid,sale.branchid as brnchid,sale_details.product_id,sale_details.mrp,sale_details.quantity,sale_details.brandid as brandid,products.cat_id,sale_details.product_id,categories.cat_id as categoryid,categories.cat_name, sum(`sale_details`.`total_price`) as `totamtss`');
					$this->db->join('sale', 'sale.id = transactions.trans_id','left');
					$this->db->join('sale_details', 'sale_details.s_id = sale.id','left');
					$this->db->join('products', 'products.pid = sale_details.product_id','left');
					$this->db->join('categories', 'categories.cat_id = products.cat_id','left');
					$queryww = $this->db->get_where("transactions", ['transactions.trans_type' => 'SALE','transactions.user_id'=>$_POST['userid'],'transactions.crop_id'=>$_POST['crop_id'],'products.cat_id'=>$_POST['category'],'sale_details.brandid'=>$_POST['branchid'],'sale_details.product_id'=>$daa['product_id']]);
					$dat1www = $queryww->row_array();
					//echo $this->db->last_query();
					/*data*/

					$data[] = array("product_name"=>$dataa['pname'],'totprice'=>round($qq),'quantity'=>$daa['sqty'],'product_id'=>$daa['product_id']);
				}
		...
		if($records)
		{
			foreach($records as $value)
			{
				...
				foreach($dat1 as $daa)
				{
					...
					$querybv = $this->db->query("select *from dummy_account_sale where userid='".$params['user_id']."' and cropid='".$params['crop_id']."' and category_id='".$value['categoryid']."' and brand_id='".$daa['brandid']."' ");
				...
				$data[] = array("category"=>$value['categoryid'],'categoryname'=>$value['cat_name'],'branchid'=>$bid,'branchname'=>$bname,'bcount'=>$bbid,'totamount'=>$bamount,'totalamount'=>$sum,'bdiscount'=>$bdss,'billdate'=>$bdate);
			}
		}
