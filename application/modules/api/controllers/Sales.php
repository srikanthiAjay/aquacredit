...
class Sales extends CI_Controller 
{
	...
	{		
		...
		$posts = array('sale_id' => 'SAL'.rand(),
			...
			's_name' => ucwords($_POST["s_name"]),
			's_mobile' => $_POST["s_mobile"],
			's_address' => ucwords($_POST["s_address"]),
			's_state' => ucwords($_POST["s_state"]),
			's_pincode' => $_POST["s_pincode"],
			'b_name' => ucwords($_POST["b_name"]),
			'b_mobile' => $_POST["b_mobile"],
			'b_address' => ucwords($_POST["b_address"]),
			'b_state' => ucwords($_POST["b_state"]),
			...
			else
			{
				...
				if($_POST['load_charge']!='' && $_POST['load_charge']!=0)
				{
					$cash = array(
							...
							"amount_type"	=>	"OUT",
							"account_type"	=>	$_POST["sale_types"],
							"account_id"	=> $admin_bank_details["data"]["id"],
							"avl_bal"		=> $admin_bank_details["data"]["avail_amount"],
							"admin_id"	=>	$this->session->userdata('adminid'),
					);
					$this->Cash_model->insert($cash);
