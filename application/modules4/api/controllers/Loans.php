...
class Loans extends CI_Controller 
{
		...
		$res_userbank_details = "";
			...
			{	
				...
				if($res["status"] == "success")
				{
					if($_POST["act_types_edit"]=="bank")
					{
						$upd_account = json_decode($this->Cash_model->updateAdminAccount($_POST["admin_bank"],$_POST["loan_amt_edit"]));
						$account_amount = $upd_account->avl_bal;
					}
					else if($_POST["act_types_edit"]=="cash")
					{
						$upd_account = json_decode($this->Cash_model->updateAdminAccount($_POST["admin_cash"],$_POST["loan_amt_edit"]));
						$account_amount = $upd_account->avl_bal;
					}
					//insert transaction
					$data = array(
	...
	public function update_single_field()
	{
		...
		echo $response = $this->Loans_model->updateActivity($_POST["loan_id"],$posts);
	}
}
?>