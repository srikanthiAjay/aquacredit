...
class Purchases_model extends CI_Model
{
	...
	//Check Branch Brand
	public function checkBranchBrand($branch_id,$ap_id,$bid){
		//company_id
		$this->db->select('ap_id,company_id,status');
		$this->db->from('branch_purchase');
		$this->db->where(['branch_id'=>$branch_id,'ap_id'=>$ap_id,'company_id'=>$bid,'status'=>'C']);
		return $this->db->get()->row_array();
	}
	...
	public function getBranchProductDetailsCnt($bp_id){
		$this->db->select('*');
		$this->db->from('branch_purchase_details');
	...
	public function getBranchPurchaseDetails($params){
		$this->db->select('bpd.bpd_id,bpd.bp_id,bpd.product_id as pid,bpd.quantity,bpd.price,bpd.total_price,p.pname,p.pmrp as prod_price,p.purchase_amt,p.percentage');
	...
	public function bpSummary($params){
		$this->db->select("SUM(IF(status='P',1,0)) as pending,SUM(IF(status='C',1,0)) as approved,SUM(IF(status='PM',1,0)) as paid,SUM(IF(status='BC',1,0)) as branch_confirm,SUM(IF(status='CE',1,0)) as completed,round(SUM(IF(status='CE' OR status='PM',total_price,0)),0) as total_purchase,(SELECT count(bp_id) as bpcnt FROM branch_purchase WHERE branch_id=".$params['branch_id'].") as bpcnt");
		$this->db->from('branch_purchase');
	...
	public function apSummary($params){
		$this->db->select("SUM(IF(status='P',1,0)) as pending,SUM(IF(status='C',1,0)) as approved,SUM(IF(status='PM',1,0)) as paid,SUM(IF(status='BC',1,0)) as admin_confirm,SUM(IF(status='CE',1,0)) as completed,round(SUM(IF(status='CE' OR status='PM',total_price,0)),0) as total_purchase,(SELECT count(ap_id) as apcnt FROM admin_purchase WHERE (status='P' OR status='C' OR status='PM' OR status='CE')) as apcnt");
		$this->db->from('admin_purchase');
	...
	//Branch Purchase Count
	public function getBranchPurchaseCount($branch_id){
		$this->db->select('*');
		$this->db->from('branch_purchase');
		$this->db->where(['branch_id'=>$branch_id]);
		return $this->db->get()->row_array();
	...
	public function getProductDetails($pid){
		//Here need to get top saled products
		$this->db->select('pid,pname,cat_id,subcat_id,brand_id,pmrp,purchase_amt,percentage,qty,status');
		$this->db->from('products');
	...
	public function delProduct($bpd_id){
		$this->db->where('bpd_id',$bpd_id);
	...
	//Delete Product From Admin Purchase Details 
	public function delProductFromAPD($params){
		$this->db->where(['ap_id'=>$params['ap_id'],'product_id'=>$params['pid']]);
	...
	public function delBranchPID($bp_id){
		$this->db->where(['bp_id'=>$bp_id]);
	...
	//Delete Product From Admin Purchase 
	...
	public function getAdminBankInfo($bank_id){
		$this->db->select('*');
		$this->db->from('ac_banks');
	...
	//Brank Account Details
	public function getBrandBankInfo($bank_id){
		$this->db->select('*');
		$this->db->from('brand_bank_accounts');
		$this->db->where(['acc_id'=>$bank_id]);
		return $this->db->get()->row_array();
	}
	...
	//Add Driver Details
	public function addDriverDetails($params){
		$this->db->insert('admin_transport_details',$params);
    	return $this->db->insert_id();
	}
	//Admin End
