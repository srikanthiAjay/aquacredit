...
class Users extends CI_Controller 
{
	...
	public function getproductsfeed()
	{
				...
				$query = $this->db->get_where("transactions", ['transactions.trans_type' => 'SALE','transactions.user_id'=>$_POST['userid'],'transactions.crop_id'=>$_POST['crop_id'],'products.cat_id'=>1]);
	...
	public function getproductsdata()
	{
				...
				$this->db->join('products', 'products.pid = sale_details.product_id','left');
				$this->db->join('categories', 'categories.cat_id = products.cat_id','left');
				$query = $this->db->get_where("transactions", ['transactions.trans_type' => 'SALE','transactions.user_id'=>$_POST['userid'],'transactions.crop_id'=>$_POST['crop_id'],'products.cat_id'=>$_POST['category'],'sale_details.brandid'=>$_POST['branchid']]);
				$dat1 = $query->result_array(); 
	...
	public function getsalesdata()
	{
		...
		if($records)
		{
			foreach($records as $value)
			{
				...
				$this->db->join('categories', 'categories.cat_id = products.cat_id','left');
				$this->db->group_by('brandid');
				$query = $this->db->get_where("transactions", ['transactions.trans_type' => 'SALE','transactions.user_id'=>$params['user_id'],'transactions.crop_id'=>$params['crop_id'],'products.cat_id'=>$value['categoryid']]);
				...
				$data[] = array("category"=>$value['categoryid'],'categoryname'=>$value['cat_name'],'branchid'=>$bid,'branchname'=>$bname,'bcount'=>$bbid,'totamount'=>$bamount,'totalamount'=>$sum);
			}
		}
