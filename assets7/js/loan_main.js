$(document).ready(function() {
    ...
    $("#skey_edit").autocomplete({
        ...
        select: function(event, ui) {
            ...
            if (ui.item.user_type == "NON_FARMER") {
                $(".sel_loc_edit").hide();
                $("#hid_user_type").val("NON_FARMER");
                //$("input[name='crop_opt_edit']").removeClass('required');
            } else {
                $(".sel_loc_edit").show();
                $("#hid_user_type").val("FARMER");
                //$("input[name='crop_opt_edit']").addClass('required');
            }

            $(".crop_val").text('Crop Location');
            $(".bank_val").text('Select User Bank');
            getusercrops(ui.item.user_id, 'edit');
            getuserbanks(ui.item.user_id, 'edit');
    ...
    $("#loanfrm").submit(function(e) {
        ...
        messages: {
            skey: {
                required: "Select registerd user",
                user_check: "Select registerd user",
            },
            crop_opt: {
    ...
    $("#loanfrm_edit").submit(function(e) {
        ...
        rules: {
            ...
            crop_opt_edit: {
                required: function(element) {
                    return ($("#hid_user_type").val() == "FARMER") ? true : false;
                }
            },
            bank_opt_edit: {
                required: function(element) {
                ...
                success: function(response) {
                    ...
                    if (res.status == 'success') {
                        ...
                        if ($("#endis").val() == 1) {
                            ...
                            //succ = "Loan details updated successfully!"; 
                            /* new PNotify({
                                title: 'Success',
                                text: succ,
                                type: 'success',
    ...
    $(".approv_btn").click(function() {
        if ($("#endis").val() == "1") {
            new PNotify({
                title: 'Error',
                text: "Please save the above form before confirming loan",
                type: 'failure',
                shadow: true,
                delay: 3000,
                stack: { "dir1": "down", "dir2": "right", "push": "top" }
            ...
            if (user_id != "") {
                ...
                if (res.data.length > 0) {
                    ...
                    $.each(res.data, function(index, crop) {
                        if (crop.cd_id == hidcrop) {
                            sel = "checked";
                            $(".crop_val").text(crop.crop_location);
                        } else { sel = ""; }
            ...
            if (user_id != "") {
                ...
                if (res.data.length > 0) {
                    ...
                    $.each(res.data, function(index, bank) {
                        ...
                        if (bank.acc_id == hidbank) {
                            sel = "checked";
                            $(".bank_val").text(bank.account_no);
                        } else { sel = ""; }
