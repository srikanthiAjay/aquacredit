...
class Users extends CI_Controller 
{
	...
	public function getproductsdata()
	{
				...
				foreach($dat1 as $daa)
				{
					...
					$data[] = array("product_name"=>$dataa['pname'],'totprice'=>$totsub,'quantity'=>$daa['sqty'],'product_id'=>$daa['product_id'],'prodiscount'=>$dataan1['product_discount'],'sids'=>$sids,'proamount'=>$qq,'purchaseamountval'=>$dataann1['purchase_amt']);
				}
		...
		if($records)
		{
			foreach($records as $value)
			{
				...
				foreach($dat1 as $daa)
				{
					...
					$queryb1 = $this->db->query("select MIN(pmrp) as productmrp from products where brand_id='".$daa['brandid']."'  ");
				...
				$data[] = array("category"=>$value['categoryid'],'categoryname'=>$value['cat_name'],'branchid'=>$bid,'branchname'=>$bname,'bcount'=>$bbid,'totamount'=>$bamount,'totalamount'=>$sum,'bdiscount'=>$bdss,'billdate'=>$bdate,'sids'=>$siddss,'productmrp'=>$ppmrp);
			}
		}
		...
		if($drawal_final["status"] == "success")
		{
			...
			if($_POST["act_types"]=="bank" || $_POST["act_types"]=="cash")
			{
				if($_POST["act_types"]=="bank")
				{
					...
					$upd_account = json_decode($this->Loans_model->updateAdminAccount($_POST["admin_bank"],$_POST["drawal_amt"]));
					$account_amount = $upd_account->avl_bal;
				}
				else if($_POST["act_types"]=="cash")
				{
					$acc_id = $_POST["admin_cash"];
					$upd_account = json_decode($this->Loans_model->updateAdminAccount($_POST["admin_cash"],$_POST["drawal_amt"]));
					$account_amount = $upd_account->avl_bal;
				}
				$cash = array(
